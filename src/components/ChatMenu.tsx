  ...
  const callChatbotAPI = async (question: string) => {
    ...
    for (const payload of payloadFormats) {
      try {
        ...
        // Extract the response content based on common API response formats
        return (
          data.response ||
          data.message ||
          data.answer ||
      ...
      <div className="flex h-full w-full flex-col">
        ...
        <div className="flex h-12 w-full items-center justify-between bg-[#12372A] px-4 py-3 text-center shadow-lg md:shadow-md">
          ...
          <button
            onClick={onClose}
            id="exit"
            className="group flex h-auto w-auto cursor-pointer items-center justify-center rounded-full p-2 transition-colors duration-300 ease-in-out hover:bg-[hsl(159,48%,18%)]"
            title="Tutup jendela"
          >
            <svg
              className="h-full w-full shrink-0 fill-[#3BAB84] group-active:scale-95"
              ...
              <div key={index} className={`w-full border-b border-b-[#00000020] py-4 last:border-b-0`}>
                <div className={`w-auto max-w-[87%] md:max-w-[85%] ${message.isBot ? "self-start" : "ml-auto"}`}>
                  <div className={`flex w-full items-start gap-2 ${!message.isBot && "flex-row-reverse"}`}>
                    <img
                      src={message.isBot ? FloraBot : User}
                      alt={`${message.isBot ? "FloraBot" : "User"} icon`}
                      width={36}
                      height={36}
                    />
                    <div className={`flex flex-col ${!message.isBot ? "items-end" : "items-start"}`}>
                      <div
                        className={`px-4 py-2 text-[#FBFADA] ${message.isBot ? "rounded-tr-[20px] rounded-br-[20px] rounded-bl-[20px] bg-[#436850]" : "rounded-tl-[20px] rounded-br-[20px] rounded-bl-[20px] bg-[#12372A]"}`}
                      >
                        {message.isBot ? (
                          ...
                          <p className="text-sm leading-relaxed [overflow-wrap:break-word] [word-break:break-word] whitespace-pre-wrap">
                            {message.content}
                          </p>
                        )}
                      </div>
          ...
          <div className="flex flex-wrap justify-start gap-2 border-b border-b-[#00000050] p-4">
            {suggestions.map((suggestion, index) => (
              <button
                ...
                className="cursor-pointer rounded-full bg-[#436850] px-4 py-2 text-sm text-[#FFF1D1] transition-colors hover:bg-[#12372A]"
                title={suggestion}
              >
                {suggestion}
              </button>
          ...
          <div id="prompt" className="sticky bottom-0 left-0 z-[20] h-24 w-full bg-white px-4 max-md:h-28 max-sm:h-36">
            <form onSubmit={handleSubmit} className="flex h-full w-full items-start justify-between gap-2 pt-6">
              <input
                ...
                className="w-full rounded-full bg-white px-4 py-3 text-sm ring-1 ring-[#12372A] outline-none ring-inset"
              ...
              <button
                ...
                className={`group flex h-auto w-auto cursor-pointer items-center justify-center rounded-full p-3 transition-[background-color,shadow,scale] duration-300 ease-in-out will-change-[background-color,box-shadow] hover:scale-[1.050] active:scale-100 ${
                  inputText.trim()
                    ? "bg-[#12372A] hover:bg-[hsl(159,51%,10%)] hover:shadow-lg"
                    : "pointer-events-none cursor-not-allowed bg-zinc-400"
                }`}
              >
                <svg
                  className="fill-[#ffffff] group-hover:fill-[#ffffff]"
                  xmlns="http://www.w3.org/2000/svg"
                  height="26px"
                  viewBox="0 -960 960 960"
                  width="26px"
